...
from core.nexus_client import NexusAPIClient # P3-T04 Integration
...
class InferenceResponse(BaseModel):
    ...
    confidence: float
    message: str
    synapse_payload: dict

class VoiceLoopRequest(BaseModel):
    audio_data_base64: str
    user_trust_level: int = 0
    hub_id: str = "hub-1" # Extracted from context in reality
...
async def voice_loop(request: VoiceLoopRequest):
    ...
    if transcription_result["confidence"] < 0.60:
         return VoiceLoopResponse(tts_audio_base64="", transcription="", latency_ms=int((time.time() - start_time) * 1000), synapse_payload={}, is_fulfillable=False)
    ...
    # P3-T04: Verify inventory availability mapped by Intent
    skus_identified = ["item-123"] # In reality, parsed by LLM
    fulfillment_status = nexus_client.verify_inventory(request.hub_id, skus_identified)
    ...
    synapse_payload = {
        ...
        "metadata": {
            ...
            "pof": fulfillment_status.get("overallPoF", 0.0)
        },
        "trigger_ms": 300
    }

    # 3. TTS
    tts_text = inference_result["action"]
    if not fulfillment_status.get("isFulfillable", False):
         tts_text = "I'm sorry, I can't fulfill all those items from your local hub right now."

    tts_audio = tts_engine.generate_speech(tts_text)
